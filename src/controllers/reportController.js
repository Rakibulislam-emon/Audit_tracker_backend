...
export const generateReport = async (req, res) => {
  try {
    ...
    const metrics = {
      ...
      criticalProblems: problems.filter((p) => p.riskRating === "critical")
        .length,
      highRiskProblems: problems.filter((p) => p.riskRating === "high").length,
      mediumRiskProblems: problems.filter((p) => p.riskRating === "medium")
        .length,
    ...
    const executiveSummary = `This report summarizes the audit conducted on ${
      ...
      ${metrics.totalProblems} issues were identified, including ${
      metrics.criticalProblems
    } critical, 
      ${metrics.highRiskProblems} high, ${
      metrics.mediumRiskProblems
    } medium, and ${metrics.lowRiskProblems} low risk issues.`;
    ...
    const newReport = new Report({
      ...
      } covering ${auditSession.checkType?.name || "various"} aspects.`,
      executiveSummary,
      findings,
      metrics,
      reportType: "final",
...
export const updateReport = async (req, res) => {
  try {
    const { title, summary, executiveSummary, findings, reportType, status } =
      req.body;
    ...
    if (findings && findings.length > 0) {
      const metrics = {
        ...
        criticalProblems: findings.filter((f) => f.riskLevel === "critical")
          .length,
        highRiskProblems: findings.filter((f) => f.riskLevel === "high").length,
        mediumRiskProblems: findings.filter((f) => f.riskLevel === "medium")
          .length,
...
export const getReportStatistics = async (req, res) => {
  try {
    ...
    const draftReports = await Report.countDocuments({ status: 'generating' });
